...
export const HabboProfilePicture: React.FC<HabboProfilePictureProps> = memo(
  ({
    ...
    const handleImageError = () => {
      const loadTime = Date.now() - loadStartTime;
      console.log(
        `❌ Erro ao carregar avatar de ${username} após ${loadTime}ms`,
      );
      setImageError(true);
    ...
    if (imageError) {
      return (
        ...
        >
          ...
          {sizePixels > 32 && (
            <div className="text-xs mt-0.5">{username.substring(0, 6)}</div>
          )}
        </div>
      );
    }
    ...
    return (
      <div
        className={`relative ${sizeClasses} ${!imageLoaded ? "bg-gray-100" : "bg-transparent"} rounded-md overflow-hidden habbo-avatar`}
      >
        {/* Loading indicator */}
