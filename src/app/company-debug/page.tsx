...
export default function CompanyDebugPage() {
  ...
  const handleTestAPI = async () => {
    ...
    try {
      // Buscar perfil com requestingUserEmail
      const response = await fetch(`/api/profiles?email=${encodeURIComponent(user.email)}&requestingUserEmail=${encodeURIComponent(user.email)}`);
  ...
  const handleClearCache = () => {
    ...
    console.log('Cache limpo para:', keys);
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-md p-8">
            ...
            <div className="mb-8 p-4 bg-green-50 dark:bg-green-900/30 rounded-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <strong>EFB:</strong> 
                  <span className={permissions.isEFB ? 'text-green-600' : 'text-red-600'}>
                    {permissions.isEFB ? ' ✅' : ' ❌'}
                  </span>
                </div>
                <div>
                  <strong>SUP:</strong> 
                  <span className={permissions.isSUP ? 'text-green-600' : 'text-red-600'}>
                    {permissions.isSUP ? ' ✅' : ' ❌'}
                  </span>
                </div>
                <div>
                  <strong>COR:</strong> 
                  <span className={permissions.isCOR ? 'text-green-600' : 'text-red-600'}>
                    {permissions.isCOR ? ' ✅' : ' ❌'}
                  </span>
                </div>
                <div>
                  <strong>Acesso Total:</strong> 
                  <span className={permissions.hasFullAccess ? 'text-green-600' : 'text-red-600'}>
              ...
              {permissions.userCompanies.length > 0 ? (
                <div className="space-y-2">
                  {permissions.userCompanies.map(company => (
                    <div key={company.id} className="p-2 bg-white dark:bg-gray-700 rounded border">
                      <div><strong>Nome:</strong> {company.nome}</div>
                      <div><strong>Sigla:</strong> {company.sigla}</div>
            ...
            <div className="mb-8 p-4 bg-purple-50 dark:bg-purple-900/30 rounded-lg">
              ...
              {Object.keys(localStorage).filter(key => key.startsWith('profile_')).length === 0 && (
