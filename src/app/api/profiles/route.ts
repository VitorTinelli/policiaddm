...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      cursosComDetalhes.forEach((curso) => {
        historico.push({
          ...
          dataFormatada: new Date(
            `${curso["dataAplicação"]}T${curso["horaAplicação"]}`,
    ...
    if (
      promocoesPunicoesComPromotor &&
      promocoesPunicoesComPromotor.length > 0
    ) {
    ...
    if (requestingUserEmail) {
      // Verificação básica
      isOwnProfile =
        militar.email === requestingUserEmail ||
      ...
      if (!isOwnProfile && requestingUserEmail.includes("@")) {
        try {
          ...
          if (militarByEmail && militarByEmail.id === militar.id) {
            isOwnProfile = true;
            console.log(
              `[API] Match encontrado por email alternativo: ${requestingUserEmail} -> ${militar.nick}`,
            );
          }
        } catch (error) {
          console.error(
            "[API] Erro ao buscar militar por email alternativo:",
            error,
          );
        }
      }
    ...
    console.log(
      `[API] Verificação de próprio perfil: requestingUserEmail=${requestingUserEmail}, militar.email=${militar.email}, militar.nick=${militar.nick}, isOwnProfile=${isOwnProfile}`,
    ); // Buscar informações de companhia se for o próprio perfil
    ...
    if (isOwnProfile) {
      try {
        const { data: companhiasData, error: companhiasError } = await supabase
          ...
          .eq("militarfk", militar.id);
        if (!companhiasError && companhiasData) {
          ...
          console.log(
            `[API] Companhias carregadas para próprio perfil ${militar.nick}: ${companhias.length}`,
          );
        } else {
    ...
    const filteredMilitar = isOwnProfile
      ? militar
      : {
          ...militar,
          email: undefined,
    ...
    console.log(
      `[API] Perfil ${isOwnProfile ? "próprio" : "de outro usuário"} solicitado: ${militar.nick}`,
    );
