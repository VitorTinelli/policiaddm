...
export async function GET(request: NextRequest) {
  try {
    ...
    let companyQuery = supabase.from("companhias").select("id, sigla, nome");
...
async function handleCompanyCourse(body: CompanyCourseRequestBody) {
  ...
  // Inserir curso
  const { error: insertCourseError } = await supabase.from("cursos").insert([
    {
      curso: courseId,
...
async function handleEFBCourse(body: EFBRequestBody) {
  const { courseName, courseStudent, courseDate, courseTime, instructorEmail } =
    body;

  if (
    typeof courseName !== "string" ||
    !courseName.trim() ||
    typeof courseStudent !== "string" ||
    !courseStudent.trim() ||
    typeof courseDate !== "string" ||
    !courseDate.trim() ||
    typeof courseTime !== "string" ||
    !courseTime.trim() ||
