...
import { getCourseScript as getScript, hasCourseScript } from '../courseScripts';
...
export default function CompanyScriptsPage() {
  ...
  const availableCompanies = {
    ...
    'EFC': 'Escola de Formação de Comandos',
    'EFF': 'Escola de Formação de Fuzileiros',
    'EFA': 'Escola de Formação de Atiradores',
    'EFP': 'Escola de Formação de Policiais',
    'CFO': 'Centro de Formação de Oficiais',
    'CID': 'Centro de Investigação e Defesa',
    'CAE': 'Centro de Apoio e Estratégia'
  };
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/courses?company=${companhia.toUpperCase()}`);
 ...
 const generateScript = useCallback((course: Course) => {
  ...
  const companyName = availableCompanies[companhia?.toUpperCase() as keyof typeof availableCompanies];
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-md p-6 mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-300">
              {companyName} - Selecione um curso para gerar o script de aplicação.
            </p>
          </div>
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-neutral-800 rounded-lg shadow-md p-6">
                ...
                {loadingScript && (
                  <div className="text-center py-12">
                    <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mb-4"></div>
                    <p className="text-gray-600 dark:text-gray-300">Gerando script...</p>
                  </div>
                    ...
                    {script === 'COURSE_SCRIPT' && selectedCourse && (
                      <div className="space-y-3">
                        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                          ...
                          {getScript(selectedCourse.sigla).map((phrase, index) => (
                            <div key={index} className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-neutral-700 rounded border">
                              <div className="flex-grow text-sm text-gray-900 dark:text-gray-100 font-mono">
                                {phrase || <span className="text-gray-400 italic">(linha vazia)</span>}
                              </div>
                              <button
                    ...
                    {script !== 'COURSE_SCRIPT' && selectedCourse && (
                      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                        ...
                        <p className="text-yellow-800 dark:text-yellow-200 text-sm">
                          Este curso ainda não está disponível no sistema. Acesse o fórum para aplicar a aula.
                        </p>
                      </div>
                    )}
                  </div>
